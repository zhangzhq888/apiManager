...
<?php if($act != 'api' && $act != 'sort'){
    $list = select('select * from project where isdel=0 order by addtime desc');
?>
    <div class="form-group">
        <input type="text" class="form-control" id="searchcate" onkeyup="searchp('project',this)" placeholder=搜索项目>
    </div>
    <div class="list">
        <ul class="list-unstyled">
            <?php foreach($list as $v){ ?>
            <form action="?act=project" method="post">
            <li class="menup" id="infop_<?php echo $v['id'];?>">
                <a href="<?php echo U(array('act'=>'show','id'=>$v['id']))?>">
                    <?php echo $v['project_name']?>
                </a>
                <br>
                <?php echo '&nbsp;&nbsp;&nbsp;&nbsp;'.$v['cdesc'];echo "<input type='hidden' name='aid' value='{$v['id']}'>";?>
                <br>
                <?php if(is_supper()){?>
                <!--只有超级管理员才可以对分类进行操作-->
            ...
            <span class="keywordp" id="<?php echo $v['id'];?>" style="display: none"><?php echo $v['cdesc'].'<|-|>'.$v['project_name'];?></span>
    ...
    <form action="?act=project" method="post">
        ...
        <div style="float:right;margin-right:20px;">
            <button class="btn btn-success" name="op" value="add">新建项目</button>
        </div>
        <?php } ?>
    </form>
<?php } else{
    ...
    $list = select($sql);?>
    <div class="form-group">
        <input type="text" class="form-control" id="searchapi" placeholder="搜索接口" onkeyup="searchp('api',this)">
    </div>
    <div class="list">
    ...
    function searchp(type,obj){
        ...
        if(type == 'cate'){//对接口分类进行搜索操作
            ...
            if($find != ''){
                $(".menup").hide();
                //找到符合关键字的对象
                var $keywordobj = $(".keywordp:contains('"+$find+"')");
//                console.log($keywordobj);
                ...
                $(".menup").show();//在没有搜索内容的情况下,左侧导航菜单 全部 显示
            }
        }
    }
