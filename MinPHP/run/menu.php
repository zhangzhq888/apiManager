...
<?php if($act != 'api' && $act != 'sort'){
    $list = select("select * from cate where isdel=0 and pid =".$_GET['id']." order by addtime desc");
?>
    <div class="form-group">
        <input type="text" class="form-control" id="searchcate" onkeyup="search('cate',this)" placeholder="搜索分类">
    </div>
    <div class="list">
    ...
    <form action="?act=cate&pid=<?php echo $_GET['id'] ?>" method="post">
        <?php if(is_supper()){?>
        <!--只有超级管理员才可以添加分类-->
